...
- A successful refresh-token exchange is not enough by itself; if the resulting token still returns `401` on live GBP `reviews.list`, treat the credential pair as wrong for the SOS GBP location and do not start the scheduler soak.
- Scheduler soak may start only after both API and worker return `200` for live `reviews.list` on the SOS tenant with the same resolved token material; token refresh success alone is not sufficient.
- If the 24-hour shadow soak closes cleanly, keep `GBP_POLL_SCHEDULER_DISABLED=0` in shadow mode and treat bare env-token `401` responses as diagnostic noise only when the compiled `GbpClient` refresh path and `gbp-poll-trigger` JobRuns remain successful.
- Recommendation-only production lock means the scheduler stays on, `POSTMARK_SEND_DISABLED=1` stays set, `REVIEW_ALERT_INBOUND_ENABLED=0` stays set, and any public reply suggestion remains a manual operator copy task outside BlackBolt.
- Minimal ops hardening means one read-only review-loop check command, one panic-disable path for `GBP_POLL_SCHEDULER_DISABLED`, and one runbook; do not introduce new monitoring vendors or a heavy alerting stack just to observe the scheduler.
- The review-loop check must execute inside `railway ssh` on a deployed service shell because the production Postgres host is private; `railway run` from the workstation cannot reach it.
- OAuth Playground identity recovery may require a manual Google password or security challenge; if headless browser automation reaches a Google sign-in prompt, stop and request the operator's fresh matched OAuth pair instead of trying to brute-force the flow.
- When debugging GBP token identity, compare both API and worker against the same live fingerprint and the `reviews.list` result. `accounts.*` 404s in this environment are not sufficient to classify a bad review token if `reviews.list` is `200` from both runtimes.
