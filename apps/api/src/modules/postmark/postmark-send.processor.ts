...
export class PostmarkSendProcessor extends WorkerHost implements OnModuleInit {
  ...
  async process(job: Job<PostmarkSendJobPayload>): Promise<void> {
    ...
    if (shouldSimulate) {
      ...
      await this.markQueueSent(campaignMessage.campaignRunId);
      await this.refreshCampaignRun(campaignMessage.campaignRunId);
      return;
    }
  ...
  private async markQueueSent(campaignRunId: string | null) {
    if (!campaignRunId) {
      return;
    }
    const now = new Date();
    await this.prisma.reviewQueueItem.updateMany({
      where: {
        campaignRunId,
        state: { in: ['scheduled', 'awaiting_approval'] }
      },
      data: {
        state: 'sent',
        sentAt: now,
